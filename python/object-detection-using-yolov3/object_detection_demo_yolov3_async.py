...
def main():
    ...
    yolov3_counter = cv2.VideoWriter(os.path.join("results"+ "/" + args.device ,"cars.mp4"), cv2.VideoWriter_fourcc(*"AVC1"), fps, (int(initial_w), int(initial_h)), True)
    result_file = open(os.path.join("results"+ "/" + args.device, 'output.txt'), "w")
    pre_infer_file = os.path.join("results"+ "/" + args.device, 'pre_progress.txt')
    infer_file = os.path.join("results"+ "/" + args.device, 'i_progress.txt')
    post_infer_file = os.path.join("results"+ "/" + args.device, 'post_progress.txt')
