...
def predict(index, img_no, plot_result):
    """
    ...
    if onnx:
        ...
        if (args.remove_training_layers == True):
            tf.keras.backend.set_learning_phase(0)
        start_time = time.time()
        pred_mask = model.predict(img, verbose=0, steps=None)
       	end_time = (time.time()-start_time) 
        #print ("Time for prediction TF: ", '\033[1m %.0f \033[0m'%(end_time*1000),"ms")
    # Plot the outcome every Nth image
    if (index % args.output_frequency == 0):
        plotDiceScore(img_no,img,msk,pred_mask,plot_result, round(end_time*1000))
    return end_time
...
for idx in indicies_validation:
    infer_time_idx = predict(val_id, idx, plot_result=True)
    infer_time += infer_time_idx
    print("Processed image " + str(idx) + " in " + str(round(infer_time_idx*1000,2)) + " ms.")
    progressUpdate(progress_file_path, time.time()-infer_time_start, val_id, len(indicies_validation)) 
    val_id += 1
