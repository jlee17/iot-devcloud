...
void placeBoxes(Mat& frame, vector<string> obj, bool is_async_mode = true){
    ...
    string count=obj[8];//people count
        //Scalar color = (std::min(int(class_id * 12.5), 255), std::min(int(class_id * 7), 255), std::min(int(class_id * 5), 255));
        Scalar color = (200, 10, 10);
	String inf_time_message;
    String current_count_message="Current count: "+ count;
        String async_mode_message;

	rectangle(frame, Point(xmin, ymin), Point(xmax, ymax), Scalar(0, 0, 255), 1 );
        putText(frame, current_count_message, Point(15, 15), CV_FONT_HERSHEY_COMPLEX, 0.6, Scalar(255, 0, 0),1);
	return; 
}
...
{	
	...
	if(cap.isOpened()){
		...
		outVideo.open(output_result, outVideo.fourcc('a','v','c','1'), int(cap.get(CAP_PROP_FPS))/skip_frame, Size(width*resl, height*resl), true);
	}
