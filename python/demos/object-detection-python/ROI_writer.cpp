...
{	
	...
	job_id = job_id.substr(0, job_id.find('.'));
	cout<<job_id<<endl;
	string input_stream = argv[1];
        string input_data = string(argv[2])+job_id+"/output.txt";
	string progress_data = string(argv[2])+job_id+"/post_progress.txt";
	string output_result = string(argv[2])+job_id+"/output.mp4";
	cout<<input_data<<" "<<progress_data<<" "<<output_result<<endl;
