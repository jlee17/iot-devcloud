...
def main():
    ...
    initial_w = cap.get(3)
    initial_h = cap.get(4)
    interact_counter = cv2.VideoWriter(os.path.join(args.output_dir, "face.mp4"), cv2.VideoWriter_fourcc(*"AVC1"), fps, (int(initial_w), int(initial_h)), True)
    ...
    progressUpdate(total_file, (sum(total_times)/1000), framesCounter+1, 1718)
